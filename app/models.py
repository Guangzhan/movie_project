...
app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql://root:222850@127.0.0.1:3306/movie"
app.configp["SQLALCHEMY_TRACK_MODIFICATIONS"] = True
...
class User(db.Model):
    ...
    addtime = db.Column(db.DateTime, index=True, default=datetime.utcnow)  # 注册实践
...
class Previesw(db.Model):
    ...
    title = db.Column(db.String(255), unique=True)  # 标题
    logo = db.Column(db.String(255), unique=True)  # 封面
    addtime = db.DateTime(db.DateTime, index=True, datetime=datetime.utcnow)  # 添加时间
